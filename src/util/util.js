...
//跳转大屏登录页
export const redirectProtalLoginPage = () => {
    // if(process.env.NODE_ENV == 'development') {
    //     router.push({ path: '/login' })    
    // } else {
    //     window.location.replace('https://jbms.nanjingdata.cn/portal/login');
    // }
    // process.env.NODE_ENV == 'development'? router.push({ path: '/login' }) : redirectProtalLoginPage()
    window.location.replace('https://jbms.nanjingdata.cn/portal/login');
}
/**
 * 加密处理
...
//canvas 图片回显
export const showCanvasImg = (ctx, imgUrl, width) => {
    var img = new Image();
    img.src = imgUrl;
    img.crossOrigin = "Anonymous";
    img.onload = () => {
        ctx.drawImage(img, 0, 0, width, 180);
    };
};
/**
 * esc监听全屏
