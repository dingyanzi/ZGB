<template>
  <div>
    <div style="width: 95%; margin: 0 auto; margin-top: 10px">
      <avue-crud :option="materialOption" :data="materialData">
...
export default {
  ...
  methods: {
    submitFn() {
      this.baoLoad = true;
      let ctFile = this.materialData.map((item) => {
        if (item.url) {
          let arr = [];
    ...
    rowDel(_row, _index) {
      this.materialData.splice(_index, 1);
    },
    subfirmFn(formName) {
    ...
    sumUpload() {
      ...
      this.materialData = this.materialData.map((item) => {
