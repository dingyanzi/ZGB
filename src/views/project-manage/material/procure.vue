<template>
  <div>
    <div style="width: 95%; margin: 0 auto; margin-top: 10px">
      <avue-crud :option="materialOption" :data="procureData">
...
export default {
  ...
  methods: {
    submitFn() {
      this.proload = true;
      let ctFile = this.procureData.map((item) => {
        if (item.url) {
          let arr = [];
    ...
    rowDel(_row, _index) {
      this.procureData.splice(_index, 1);
    },
    handle(row) {
      this.currenttab = row;
      const { url } = row;
    ...
    sumUpload() {
      ...
      this.procureData = this.procureData.map((item) => {
