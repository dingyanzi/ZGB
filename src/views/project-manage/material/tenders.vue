<template>
  <div>
    <div style="width: 95%; margin: 0 auto; margin-top: 10px">
      <avue-crud :option="materialOption" :data="tendersData">
...
export default {
  ...
  methods: {
    submitFn() {
      this.tenLoad = true;
      let ctFile = this.tendersData.map((item) => {
        if (item.url) {
          let arr = [];
    ...
    rowDel(_row, _index) {
      this.tendersData.splice(_index, 1);
    },
    handle(row) {
      this.currenttab = row;
      const { url } = row;
    ...
    sumUpload() {
      ...
      this.tendersData = this.tendersData.map((item) => {
