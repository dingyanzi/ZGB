<template>
  <div>
    <div style="width: 95%; margin: 0 auto; margin-top: 10px">
      <avue-crud :option="materialOption" :data="concluData">
...
export default {
  ...
  methods: {
    submitFn() {
      this.conLoad = true;
      let ctFile = this.concluData.map((item) => {
        if (item.url) {
          let arr = [];
    ...
    rowDel(_row, _index) {
      this.concluData.splice(_index, 1);
    },
    handle(row) {
      this.currenttab = row;
      const { url } = row;
    ...
    sumUpload() {
      ...
      this.concluData = this.concluData.map((item) => {
