<template>
  <div>
    <div style="width: 95%; margin: 0 auto; margin-top: 10px">
      <avue-crud :option="materialOption" :data="acceptData">
...
export default {
  ...
  methods: {
    submitFn() {
      this.accLoad = true;
      let ctFile = this.acceptData.map((item) => {
        if (item.url) {
          let arr = [];
    ...
    rowDel(_row, _index) {
      this.acceptData.splice(_index, 1);
    },
    handle(row) {
      this.currenttab = row;
      const { url } = row;
    ...
    sumUpload() {
      ...
      this.acceptData = this.acceptData.map((item) => {
